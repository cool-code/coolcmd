...
4. **路径自适应：** 安装脚本会自动查询 `HKCU\Software\Clink` 注册表键值，无论 Clink 安装在 `C:\Program Files` 还是 `Program Files (x86)`，都能精确定位 `clink.bat`。
5. **静默安装：** 使用了 `--source winget` 与 `autorun install -- -q`，尽量减少弹窗干扰。
6. **清理自毁：** 脚本执行完毕后会启动一个后台进程自我删除 ` (del "%~f0")`，保持用户下载目录整洁。
